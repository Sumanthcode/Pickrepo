...
public class A_EquityOptionCallPage {
	
	@FindBy(xpath = "//input[@formcontrolname='scriptName']")
	public WebElement scripNameField;
	
	@FindBy(xpath = "//span[contains(text(),'WIPR')]")
	public List<WebElement> EquityOptionAutoSuggesions;
	...
	public A_EquityOptionCallPage(WebDriver driver)
	{
	...
	public String getEquityOptionAutoSuggesions() throws InterruptedException, AWTException {
		Thread.sleep(2000);
		Robot r = new Robot();
		r.keyPress(KeyEvent.VK_BACK_SPACE);
		Thread.sleep(2000);
		int count=0;
		String scripName = EquityOptionAutoSuggesions.get(count).getText();
		EquityOptionAutoSuggesions.get(count).click();
		count++;
		if(count==2)
		{
			count=0;
